...
function EtherDex() {
  ...
  this.contractEtherDex = undefined;
...
EtherDex.prototype.dialogInfo = function dialogInfo(message) {
  console.log(message);
...
EtherDex.prototype.dialogError = function dialogError(message) {
  console.log(message);
...
EtherDex.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
EtherDex.prototype.txError = function txError(err) {
  ...
  utility.getBalance(this.web3, this.addrs[this.selectedAccount], (errBalance, resultBalance) => {
    ...
    if (this.connection.connection === 'RPC' && this.addrKinds[this.selectedAccount] !== 'MetaMask') {
      if (this.pks[this.selectedAccount]) {
        this.dialogError('You are using an EtherDex account that has a private key attached, but you\'re connected to MetaMask. You should disable MetaMask from Chrome\'s Window -> Extensions menu (don\'t worry, this won\'t lose your MetaMask data), then refresh BitDex.');
        ga('send', {
          hitType: 'event',
          eventCategory: 'Error',
...
EtherDex.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
EtherDex.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
EtherDex.prototype.createAccount = function createAccount() {
...
EtherDex.prototype.deleteAccount = function deleteAccount() {
  if (this.pks[this.selectedAccount]) {
...
EtherDex.prototype.selectAccount = function selectAccount(i) {
...
EtherDex.prototype.addAccount = function addAccount(newAddr, newPk) {
...
EtherDex.prototype.showPrivateKey = function showPrivateKey() {
  const addr = this.addrs[this.selectedAccount];
  const pk = this.pks[this.selectedAccount];
  if (!pk) {
    this.dialogError(
...
EtherDex.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
EtherDex.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractEtherDexAddr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
EtherDex.prototype.displayAccounts = function displayAccounts(callback) {
...
EtherDex.prototype.displayLanguages = function displayLanguages(callback) {
...
EtherDex.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
EtherDex.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractEtherDexAddr) {
...
EtherDex.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractEtherDexAddr &&
...
EtherDex.prototype.isInCross = function isInCross(priceIn, kind) {
...
EtherDex.prototype.displayVolumes = function displayVolumes(
...
EtherDex.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address.toLowerCase() === this.config.contractEtherDexAddr.toLowerCase()) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        let trade;
        if (
...
EtherDex.prototype.candlestickChart =
...
EtherDex.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
EtherDex.prototype.getReturnTicker = function getTopOrders(callback) {
...
EtherDex.prototype.getTopOrders = function getTopOrders(callback) {
...
EtherDex.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
  ...
  orders = orders.filter(order =>
    ...
    Number(order.ethAvailableVolumeBase).toFixed(3) >= this.minOrderSize);
  // filter only orders that match the smart contract address
  orders = orders.filter(order => order.order.contractAddr === this.config.contractEtherDexAddr);
  // final order filtering and sorting
  const buyOrders = orders.filter(x => x.amount > 0);
  const sellOrders = orders.filter(x => x.amount < 0);
  sellOrders.sort((a, b) => b.price - a.price || b.id - a.id);
...
EtherDex.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
EtherDex.prototype.displayAllBalances = function displayAllBalances(callback) {
  ...
  async.map(
    ...
    (token, callbackMap) => {
      if (token.addr === zeroAddr) {
        utility.call(
          this.web3,
          this.contractEtherDex,
          this.config.contractEtherDexAddr,
          'balanceOf',
...
EtherDex.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
  ...
  } else if (toAddr.toLowerCase() === this.config.contractEtherDexAddr.toLowerCase()) {
    this.dialogError('If you send funds directly to the EtherDex smart contract, they will be lost. You need to use the Deposit tab to deposit.');
    ga('send', {
      hitType: 'event',
      eventCategory: 'Error',
      eventAction: 'Transfer - invalid address',
...
EtherDex.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = new BigNumber(Number(utility.ethToWei(inputAmount, this.getDivisor(addr))));
  const token = this.getToken(addr);
  if (amount.lte(0)) {
  ...
  if (addr.slice(0, 39) === '0x0000000000000000000000000000000000000') {
    ...
    utility.call(
      ...
      token.addr,
      'allowance',
      [this.addrs[this.selectedAccount], this.config.contractEtherDexAddr],
                  ...
                  (callbackSeries) => {
                    if (resultAllowance.eq(0)) {
                      utility.send(
                        ...
                        addr,
                        'approve',
                        [this.config.contractEtherDexAddr, amount,
...
EtherDex.prototype.withdraw = function withdraw(addr, amountIn) {
...
EtherDex.prototype.order = function order(direction, amount, price, expires, refresh) {
...
EtherDex.prototype.publishOrder = function publishOrder(
  ...
  if (direction === 'buy') {
    ...
    const amountGetEth = utility.weiToEth(amounttGe, this.getDivisor(tokenGet));
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        hash, this.pks[this.selectedAccount], (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.config.contractetherDexAddr,
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
...
EtherDex.prototype.cancelOrder = function cancelOrder(orderIn) {
...
EtherDex.prototype.trade = function trade(kind, order, inputAmount) {
  if (this.addrs[this.selectedAccount].slice(0, 39) === '0x0000000000000000000000000000000000000') {
    this.dialogError(
      "You haven't selected an account. Make sure you have an account selected from the Accounts dropdown in the upper right.");
    ga('send', {
...
EtherDex.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
EtherDex.prototype.updateUrl = function updateUrl() {
...
EtherDex.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
EtherDex.prototype.getToken = function getToken(addrOrToken, name, decimals) {
  let result;
...
EtherDex.prototype.loadToken = function loadToken(addr, callback) {
...
EtherDex.prototype.selectToken = function selectToken(addrOrToken, name, decimals) {
...
EtherDex.prototype.selectBase = function selectBase(addrOrToken, name, decimals) {
...
EtherDex.prototype.selectTokenAndBase = function selectTokenAndBase(tokenAddr, baseAddr) {
...
EtherDex.prototype.setGasPrice = function setGasPrice(gasPrice) {
  if (gasPrice) {
...
EtherDex.prototype.displayBuySell = function displayBuySell(callback) {
...
EtherDex.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
EtherDex.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
EtherDex.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
EtherDex.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
EtherDex.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractEtherDexAddrs,
    contractAddr: this.config.contractEtherDexAddr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractBitDexAddr}`,
  });
};
EtherDex.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
EtherDex.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractEtherDexAddr) &&
    (this.addrs.length > 1 ||
      (this.addrs.length === 1 && this.addrs[0].slice(0, 39) !== '0x0000000000000000000000000000000000000'))
  ) {
    this.dialogInfo(
      '<p>EtherDex has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll(); bundle.BitDex.displayHelp(\'smartContract\')">Smart contract changelog</a></p>');
  }
};
EtherDex.prototype.resetCaches = function resetCaches() {
...
EtherDex.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
EtherDex.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  this.q.push((done) => {
    if (token && base) {
    ...
    this.selectedContract = this.config.contractEtherDexAddr;
...
EtherDex.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
EtherDex.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
EtherDex.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
EtherDex.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractEtherDexAddr = this.config.contractEtherDexAddrs[0].addr;
    // get accounts
    ...
    utility.loadContract(
      this.web3,
      this.config.contractEtherDex,
      this.config.contractEtherDexAddr,
      (err, contractEtherDex) => {
        this.contractEtherDex = contractEtherDex;
        utility.loadContract(
          this.web3,
...
EtherDex.prototype.startEtherDex = function startEtherDex() {
...
const etherDex = new EtherDex();

module.exports = { EtherDex: etherDex, utility };
